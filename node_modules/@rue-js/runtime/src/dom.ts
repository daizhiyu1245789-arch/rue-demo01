...
/*
 * [RUE-MOUNT-074] 应用挂载 · 步骤 74：withDOMHostOperations(parentContext, 生成的 setup 回调)
 * 对应指南：RUE_SOURCE_GUIDE.html#flow=mount&view=sequence&node=74
 * 所在阶段：E · 返回值为 Vapor：setup 执行与子树返回；节点类型：调用。
 *   1. 如果已有 activeDOMHostOperationContext，直接运行回调，沿用当前上下文。
 *   2. 否则尝试从 parent 关联的上下文取得 adapter；没有继承值时使用 getCurrentDOMAdapter。
 *   3. 保存旧上下文和 domAdapterGeneration，建立本轮上下文，然后在 try 中执行 run()。
 * 执行结果：真正的编译 setup 在这个回调中运行，内部 DOM helper 才能使用本轮选出的宿主能力。
 * 衔接说明：在 DOM host 上下文执行传入的回调；编译生成的 setup 在这里真正创建节点、绑定属性/事件并挂载子组件。
 * 断点观察：观察 activeDOMHostOperationContext、inheritedContext、adapter 和 generation 的来源。
 * [RUE-MOUNT-075] 应用挂载 · 步骤 75：withDOMHostOperations 的 run() → 生成 setup(parentContext)
 * 对应指南：RUE_SOURCE_GUIDE.html#flow=mount&view=sequence&node=75
 * 所在阶段：E · 返回值为 Vapor：setup 执行与子树返回；节点类型：执行回调。
 *   1. run 是 wrappedSetup 传入的 () => setup(parentContext)，调用它才进入生成的 setup。
 *   2. 已有宿主上下文时函数会直接 return run()，不新建一层上下文。
 *   3. 新建上下文的分支在 finally 检查 generation 是否仍与 domAdapterGeneration 相同；没有发生代际变化才恢复 previous。
 * 执行结果：setup 的结果经 run() 逐层返回。代际检查避免把执行期间发生的适配器变化覆盖为旧上下文。
 * 衔接说明：已有宿主上下文时直接 run()；否则保存旧上下文并建立新上下文，try 中 run()，finally 恢复。
 * 断点观察：确认走直接返回还是新上下文分支，比较 generation 和 domAdapterGeneration。
 */
...
export const appendChild = (parent: DomNodeLike, child: DomNodeLike) => {
  /*
   * [11 组装 DOM 树]
   * 调用链：setup() -> _$appendChild() -> appendChild() -> parent.appendChild()。
   * 观察：parent、child、parent.childNodes。
   */
  // oxlint-disable-next-line no-debugger -- Rue 源码学习用自动断点。
  if ((globalThis as any).__RUE_RENDER_DEBUG__?.take('11.dom-append-child')) debugger
  if (activeDOMHostOperationContext?.freshBrowser) {
    ;(parent as any).appendChild(child)
    syncSelectValueForMutationParent(parent)
    return
