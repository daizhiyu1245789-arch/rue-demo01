...
export const mountExplanations091_105: MountExplanationMap = {
  "91": {
    "context": "最外层渲染回调返回，runRenderEntry 现在终于执行此前暂缓的 finally。这里对应源码节点 91 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "进入生命周期刷新函数。它完成后才回到根回调，继续应用级的 mounted 通知。执行完这一站后，调用栈继续按直线图向下走：下一节点是「flushPendingComponentLifecycle()：排序并遍历队列」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "观察 renderDepth 归零和 pendingComponentLifecycle 的长度，区分入队与实际回调执行。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/create-rue-base.js:78 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "92": {
    "context": "生命周期刷新函数取出当前批次，按节点关系排序，再在各自实例上下文中通知。这里对应源码节点 92 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "每个 Hook 都在对应组件上下文中执行，当前批次及新增待处理项完成后刷新函数才返回。执行完这一站后，调用栈继续按直线图向下走：下一节点是「lifecycle.call(instance, \"mounted\") → invoke(hooks)」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "看 pending 中的 instance、name、subtree，以及通知前组件是否仍存在于实例管理器。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/component.js:65 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "93": {
    "context": "lifecycle.call 根据当前实例和生命周期名称，找到已注册的回调集合。这里对应源码节点 93 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "进入 invoke 的循环。用户 Hook 自己再触发什么工作，要根据具体回调代码继续判断。执行完这一站后，调用栈继续按直线图向下走：下一节点是「invoke(hooks)：逐个执行 hook()」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "观察 name、hooks.get(name) 和当前实例，确认取到的是哪一组注册回调。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/lifecycle.js:58 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "94": {
    "context": "invoke 负责真正调用生命周期列表中的每个函数。这里对应源码节点 94 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "列表执行完返回 lifecycle.call，批次全部通知完返回 flushPendingComponentLifecycle。异步 Hook 的后续执行不属于这里等待完成的同步链。执行完这一站后，调用栈继续按直线图向下走：下一节点是「回到 renderRoot：lifecycle.callGlobal(\"mounted\")」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "检查 hook 的函数位置和返回值，注意没有 await 时 Promise 完成不会阻塞这个循环。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/lifecycle.js:26 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "95": {
    "context": "runRenderEntry 已完成组件生命周期刷新，根渲染回调继续执行其下一句。这里对应源码节点 95 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "进入 callGlobal，然后经 invoke 执行全局回调。之后回到应用控制器的 render(app) 调用处。执行完这一站后，调用栈继续按直线图向下走：下一节点是「callGlobal(name) → invoke(globalHooks.get(name))」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "对比组件 hooks 与 globalHooks，确认当前 name 虽相同，但取回调的容器不同。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/create-rue-base.js:147 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "96": {
    "context": "callGlobal 是一层很薄的转发：按名称取全局列表，再交给已经见过的 invoke。这里对应源码节点 96 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "应用控制器里的 const result = render(app) 接到 undefined。这不代表挂载失败，成功与否由事务和异常状态判断。执行完这一站后，调用栈继续按直线图向下走：下一节点是「回到 appController：transaction.status = \"mounted\"」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "看 render(app) 返回后的 result 与 DOM、transaction.status，避免把 undefined 误判为没有渲染。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/lifecycle.js:74 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "97": {
    "context": "根渲染回调正常返回，应用控制器开始确认本轮事务能否算成功。这里对应源码节点 97 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "事务成功状态已记录，但离开函数前还要运行 finally 恢复 activeAppMount。执行完这一站后，调用栈继续按直线图向下走：下一节点是「appController finally：恢复 activeAppMount」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "看 transaction.error、status 从 mounting 到 mounted 的变化，以及深层记录失败时的分支。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/app.js:91 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "98": {
    "context": "无论应用控制器正常返回还是抛错，finally 都会恢复挂载上下文。这里对应源码节点 98 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "包装层的 result 得到原始方法返回值，接着检查是否有待处理入口错误。执行完这一站后，调用栈继续按直线图向下走：下一节点是「回到 wrappedRuntimeEntry：检查 pending error」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "检查 activeAppMount 恢复为原值；它与已经保存在 appMounts 的成功事务不是同一个“当前值”概念。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/js-runtime/app.js:102 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "99": {
    "context": "原始 mount 已返回，但入口包装还要处理深层可能记录而尚未抛出的错误。这里对应源码节点 99 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "正常结果准备回到 useApp 的 mountRunner；异常则向外传播，由上层挂载回滚逻辑处理。执行完这一站后，调用栈继续按直线图向下走：下一节点是「wrappedRuntimeEntry finally：入口深度减一」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "看 pending、rethrowingPendingEntryError 和 result，区分原方法抛错与待处理错误被重新抛出。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/runtime-entry-wrap.js:147 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "100": {
    "context": "入口包装退出时，需要把自己增加的调用深度减掉，避免后续入口误判仍处于嵌套调用。这里对应源码节点 100 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "底层 mount 调用链已经返回，上面两层上下文包装仍要完成容器出栈和活动 runtime 恢复。执行完这一站后，调用栈继续按直线图向下走：下一节点是「runWithClientRuntime finally：popCurrentContainer()」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "观察入口深度恢复，以及调用栈从 runtime 包装层退回 useApp 箭头函数。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime-vapor/dist/runtime-entry-wrap.js:164 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "101": {
    "context": "mountRunner 返回或抛错后，contextRunner 的 finally 开始清理容器上下文。这里对应源码节点 101 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "contextRunner 结束，回到 runWithRuntime，然后执行它自己的 finally。执行完这一站后，调用栈继续按直线图向下走：下一节点是「runWithRuntime finally：恢复原活动 runtime」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "对照 didPush 的原始值，查看共享 bridge 当前容器在出栈前后的变化。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime/src/client-runtime.ts:193 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "102": {
    "context": "容器上下文已结束，最外层运行时包装恢复进入挂载前的活动 runtime。这里对应源码节点 102 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "运行时与容器的临时上下文都已恢复，外层 mount 可以继续确认应用容器。执行完这一站后，调用栈继续按直线图向下走：下一节点是「useApp.mount 继续：标记容器、确认归属」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "不仅比较 __rue_active 的值，也检查 hasOwnProperty；“没有属性”和“属性值为 undefined”不同。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime/src/runtime-context.ts:44 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "103": {
    "context": "runWithClientRuntime 已正常返回，应用管理层开始把临时挂载状态确认为正式占用。这里对应源码节点 103 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "当前应用正式持有这个容器。pendingContainerRef 还需在最后的 finally 中清除。执行完这一站后，调用栈继续按直线图向下走：下一节点是「confirmAppContainer(reservation)」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "观察 el 的 data-rue-app、reservation 和 containerRef；确认赋值发生在底层挂载返回之后。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime/src/hooks/useApp.ts:125 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "104": {
    "context": "confirmAppContainer 防止挂载过程中原预留记录失效后，仍错误地确认另一次占用。这里对应源码节点 104 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "返回 useApp.mount，后者才执行 containerRef = el。若这里抛错，上层会进入已有 catch 的卸载与回滚路径。执行完这一站后，调用栈继续按直线图向下走：下一节点是「useApp.mount finally：清除 pendingContainerRef，返回 main.ts」，它位于「F · 提交 #app → 生命周期 → 逐层返回」。",
    "watch": "比较 current.reservation 与传入 reservation 的引用，查看 confirmed 从 false 变为 true。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime/src/hooks/app-container-ownership.ts:35 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  },
  "105": {
    "context": "useApp.mount 到达最终清理阶段，本次同步首次挂载即将结束。这里对应源码节点 105 / 105，不是孤立代码，而是整条应用挂载同步调用链中的一站。",
    "steps": [
    ...
    "result": "正常路径返回 main.ts，本行链式挂载调用完成。之后的点击、微任务、路由加载和响应式更新，要从各自触发点另起一条执行链。执行完这一站后，调用栈继续按直线图向下走：这是应用挂载主题的最后一个节点，后续交给用户交互、路由异步任务或响应式更新重新触发。",
    "watch": "结束时确认 pendingContainerRef 为空、containerRef 为 el、DOM 在 #app 内；不要把后续异步回调排进本次同步返回顺序。同时建议看调用栈顶部是否从 node_modules/@rue-js/runtime/src/hooks/useApp.ts:143 进入，以及当前函数是正在执行，还是只是被创建并传递。"
  }
};
